...
const clinical_notes_1 = __importDefault(require("./routes/clinical-notes"));
const whatsappRoutes_1 = __importDefault(require("./modules/whatsapp/routes/whatsappRoutes"));
...
app.use(express_1.default.urlencoded({ extended: true })); // Para receber dados do Twilio
...
app.get('/api/service-types', auth_1.authMiddleware, async (req, res) => {
    try {
...
app.post('/api/service-types', auth_1.authMiddleware, async (req, res) => {
    try {
        const { name, description, icon, color, sortOrder } = req.body;
        if (!name) {
            return res.status(400).json({ error: 'Nome é obrigatório' });
        }
        const serviceType = await prisma_1.default.serviceType.create({
            data: {
...
app.put('/api/service-types/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, icon, color, active, sortOrder } = req.body;
        const serviceType = await prisma_1.default.serviceType.update({
            where: {
                id,
                companyId: req.user.companyId
            },
...
app.delete('/api/service-types/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.default.serviceType.delete({
            where: {
                id,
                companyId: req.user.companyId
            }
...
app.get('/api/business-types', async (req, res) => {
    try {
        const businessTypes = await prisma_1.default.businessType.findMany({
            where: { active: true },
            include: {
                tabs: {
                    where: { active: true },
...
app.get('/api/business-types/:id/tabs', async (req, res) => {
    try {
        const { id } = req.params;
        const tabs = await prisma_1.default.businessTypeTab.findMany({
            where: {
                businessTypeId: id,
                active: true
            },
...
app.get('/api/clients/:clientId/tabs', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const clientTabData = await prisma_1.default.clientTabData.findMany({
            where: { clientId },
            include: {
                tab: {
                    include: {
...
app.put('/api/clients/:clientId/tabs/:tabId', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const tab = await prisma_1.default.businessTypeTab.findUnique({ where: { id: tabId } });
        if (!tab) {
            return res.status(404).json({ error: 'Aba não encontrada' });
        }
        // Buscar dados atuais para comparação (se existirem)
        const existingTabData = await prisma_1.default.clientTabData.findUnique({
            where: {
                clientId_tabId: {
                    clientId,
                    tabId
        ...
        const clientTabData = await prisma_1.default.clientTabData.upsert({
            where: {
                clientId_tabId: {
                    clientId,
                    tabId
...
app.get('/api/clients/:clientId/tabs/:tabId/history', async (req, res) => {
    try {
        const { clientId, tabId } = req.params;
        const clientTabData = await prisma_1.default.clientTabData.findUnique({
            where: {
                clientId_tabId: {
                    clientId,
                    tabId
...
app.get('/api/company/config', auth_1.authMiddleware, async (req, res) => {
    try {
...
app.put('/api/company/business-type', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const businessType = await prisma_1.default.businessType.findUnique({
...
app.get('/api/clients/:clientId/medical-records', auth_1.authMiddleware, async (req, res) => {
    try {
        const { clientId } = req.params;
        ...
        const medicalRecords = await prisma_1.default.clientTabData.findMany({
            where: {
                clientId,
                tab: {
...
app.get('/api/clients/:clientId/service-history', auth_1.authMiddleware, async (req, res) => {
    try {
        const { clientId } = req.params;
        // Verificar se cliente pertence à empresa
        const client = await prisma_1.default.client.findFirst({
        ...
        const serviceHistory = await prisma_1.default.appointment.findMany({
            where: {
                companyId: req.user.companyId,
                clientId,
                status: 'COMPLETED'
...
app.post('/api/clients/:clientId/medical-certificate', auth_1.authMiddleware, async (req, res) => {
    try {
        const { clientId } = req.params;
        ...
        const attestTab = await prisma_1.default.businessTypeTab.findFirst({
        ...
        const existingData = await prisma_1.default.clientTabData.findUnique({
            where: {
                clientId_tabId: {
                    clientId,
                    tabId: attestTab.id
        ...
        await prisma_1.default.clientTabData.upsert({
            where: {
                clientId_tabId: {
                    clientId,
                    tabId: attestTab.id
...
// EMISSÃO DE RECEITAS
app.post('/api/clients/:clientId/prescription', auth_1.authMiddleware, async (req, res) => {
    try {
        const { clientId } = req.params;
        ...
        const prescriptionTab = await prisma_1.default.businessTypeTab.findFirst({
            where: { slug: 'receitas' }
        });
        if (!prescriptionTab) {
...
app.get('/api/clients', auth_1.authMiddleware, async (req, res) => {
    try {
...
app.get('/api/clients/search/:cpf', auth_1.authMiddleware, async (req, res) => {
    try {
...
app.get('/api/clients/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const client = await prisma_1.default.client.findFirst({
            where: {
...
// Criar novo cliente
app.post('/api/clients', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (company?.businessType?.tabs && company.businessType.tabs.length > 0) {
            ...
            const tabCreationPromises = company.businessType.tabs.map(tab => prisma_1.default.clientTabData.create({
                data: {
                    clientId: client.id,
                    tabId: tab.id,
...
// Atualizar cliente
app.put('/api/clients/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { fullName, cpf, phone, email, birthDate, notes, cep, street, number, complement, neighborhood, city, state } = req.body;
        if (!fullName || !cpf || !phone) {
...
// Criar novo serviço
app.post('/api/services', auth_1.authMiddleware, async (req, res) => {
    try {
        const { name, serviceTypeId, duration, price, description, mondayEnabled, tuesdayEnabled, wednesdayEnabled, thursdayEnabled, fridayEnabled, saturdayEnabled, sundayEnabled, startTime, endTime } = req.body;
...
// Atualizar serviço
app.put('/api/services/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, serviceTypeId, duration, price, description, mondayEnabled, tuesdayEnabled, wednesdayEnabled, thursdayEnabled, fridayEnabled, saturdayEnabled, sundayEnabled, startTime, endTime, active } = req.body;
...
// Excluir serviço
app.delete('/api/services/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/api/appointments', auth_1.authMiddleware, async (req, res) => {
    try {
...
app.post('/api/appointments/check-availability', async (req, res) => {
    try {
        ...
        const conflictingAppointments = await prisma_1.default.appointment.findMany({
            where: whereClause,
            include: {
                client: true,
                service: true
...
app.put('/api/appointments/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// Deletar agendamento
app.delete('/api/appointments/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.patch('/api/appointments/:id/confirm', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const appointment = await prisma_1.default.appointment.update({
            where: {
...
app.patch('/api/appointments/:id/cancel', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const appointment = await prisma_1.default.appointment.update({
            where: {
...
app.post('/api/appointments/:id/reschedule', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const originalAppointment = await prisma_1.default.appointment.findFirst({
            where: {
                id,
                companyId: req.user.companyId
            }
...
app.get('/api/dental-treatments', async (req, res) => {
    try {
        const { search } = req.query;
        const treatments = await prisma_1.default.dentalTreatment.findMany({
            where: {
                active: true,
                ...(search && {
                    name: {
...
app.post('/api/dental-treatments', async (req, res) => {
    try {
        const { name, category, description, estimatedDuration, averagePrice, color } = req.body;
        if (!name || !category) {
            return res.status(400).json({ error: 'Nome e categoria são obrigatórios' });
        }
        const treatment = await prisma_1.default.dentalTreatment.create({
            data: {
                name,
                category,
                description,
...
app.get('/api/clients/:clientId/tooth-treatments', auth_1.authMiddleware, async (req, res) => {
    try {
        const { clientId } = req.params;
        // Verificar se cliente pertence à empresa
        const client = await prisma_1.default.client.findFirst({
        ...
        const toothTreatments = await prisma_1.default.toothTreatment.findMany({
            where: { clientId },
            include: {
                treatment: true
            },
...
app.post('/api/clients/:clientId/tooth-treatments', auth_1.authMiddleware, async (req, res) => {
    try {
        const { clientId } = req.params;
        ...
        const treatment = await prisma_1.default.dentalTreatment.findUnique({ where: { id: treatmentId } });
        if (!treatment) {
            return res.status(404).json({ error: 'Tratamento não encontrado' });
        }
        const toothTreatment = await prisma_1.default.toothTreatment.create({
            data: {
                clientId,
...
app.put('/api/tooth-treatments/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { status, notes, datePerformed, price } = req.body;
        // Verificar se o tratamento pertence a um cliente da empresa
        const existingTreatment = await prisma_1.default.toothTreatment.findFirst({
            where: {
                id,
                client: {
                    companyId: req.user.companyId
        ...
        const toothTreatment = await prisma_1.default.toothTreatment.update({
            where: { id },
            data: {
                status,
                notes,
...
app.delete('/api/tooth-treatments/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        // Verificar se o tratamento pertence a um cliente da empresa
        const existingTreatment = await prisma_1.default.toothTreatment.findFirst({
        ...
        await prisma_1.default.toothTreatment.delete({
            where: { id }
        });
...
app.get('/api/financial/overview', auth_1.authMiddleware, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        ...
        appointments.forEach(appointment => {
            ...
            if (appointment.status === 'COMPLETED') {
                ...
                if (random < 0.4)
                    formasPagamento.cartao += valor;
                else if (random < 0.7)
                    formasPagamento.pix += valor;
                else if (random < 0.9)
                    formasPagamento.dinheiro += valor;
                else
                    formasPagamento.transferencia += valor;
            }
...
app.get('/api/financial/transactions', auth_1.authMiddleware, async (req, res) => {
    try {
...
app.get('/api/financial/services-performance', auth_1.authMiddleware, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const whereClause = {
            companyId: req.user.companyId,
...
app.get('/api/financial/cash-flow', auth_1.authMiddleware, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        // Define período padrão (últimas 4 semanas se não especificado)
        const now = new Date();
        const start = startDate ? new Date(startDate) : new Date(now.getTime() - 28 * 24 * 60 * 60 * 1000);
...
app.get('/api/appointments/:id/details', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/api/appointments/complete', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        // Cálculo de valores
        const totalService = services.reduce((sum, s) => sum + s.price, 0);
        const totalProducts = products?.reduce((sum, p) => sum + (p.salePrice * p.qty), 0) || 0;
            ...
            if (paymentStatus === 'PAID') {
                ...
                await tx.transaction.create({
                    data: {
                        ...
                        status: 'PAID',
                        type: 'SERVICE',
                        description: `${services.map((s) => s.name).join(', ')} (Pagamento Parcial)`,
                        paymentDate: new Date()
                    }
                });
                ...
                await tx.transaction.create({
                    data: {
                        ...
                        status: 'PENDING',
                        type: 'SERVICE',
                        description: `${services.map((s) => s.name).join(', ')} (Saldo Pendente)`,
                        dueDate: dueDate ? new Date(dueDate) : null
                    }
                });
            }
...
app.get('/api/clients/:clientId/history', auth_1.authMiddleware, async (req, res) => {
    try {
        const { clientId } = req.params;
        // Verificar se o cliente pertence à empresa do usuário
        const client = await prisma_1.default.client.findFirst({
        ...
        const history = await prisma_1.default.appointmentHistory.findMany({
            where: {
                clientId,
                companyId: req.user.companyId
            },
...
app.get('/api/products/financial-analysis', auth_1.authMiddleware, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const whereClause = {
            companyId: req.user.companyId,
        ...
        const productTransactions = await prisma_1.default.transaction.findMany({
            where: whereClause,
        ...
        const products = Object.values(productAnalysis).sort((a, b) => b.profit - a.profit);
...
app.get('/api/financial/summary', auth_1.authMiddleware, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const now = new Date();
        ...
        const services = await prisma_1.default.transaction.aggregate({
            where: { ...transactionWhereClause, type: 'SERVICE' },
            _sum: { amount: true }
        });
        const products = await prisma_1.default.transaction.aggregate({
        ...
        if (startDate && endDate) {
            appointmentsWhereClause.date = {
                gte: new Date(Math.max(now.getTime(), new Date(startDate).getTime())),
                lte: new Date(endDate)
            };
        }
        const futureAppointments = await prisma_1.default.appointment.aggregate({
        ...
        if (startDate && endDate) {
            overdueWhereClause.date = {
                gte: new Date(startDate),
                lt: new Date(Math.min(now.getTime(), new Date(endDate).getTime()))
            };
        }
        const overdueAppointments = await prisma_1.default.appointment.aggregate({
...
app.post('/api/transactions/:id/pay', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/api/debug/samuel', auth_1.authMiddleware, async (req, res) => {
    try {
        const appointments = await prisma_1.default.appointment.findMany({
            where: {
                companyId: req.user.companyId,
...
app.use('/api', auth_1.authMiddleware, clinical_notes_1.default);
app.listen(PORT, () => {
