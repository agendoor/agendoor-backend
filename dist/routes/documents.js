...
const createThumbnail = async (filePath, originalName) => {
    try {
        ...
        const thumbnailName = `thumb-${path_1.default.basename(filePath, path_1.default.extname(filePath))}.webp`;
...
router.get('/clients/:clientId/documents', async (req, res) => {
    try {
        ...
        const documentsWithUrls = documents.map(doc => ({
            ...
            url: `/uploads/documents/${path_1.default.basename(doc.filePath)}`,
...
router.post('/clients/:clientId/documents', upload.array('files', 10), async (req, res) => {
    try {
        const { clientId } = req.params;
...
router.put('/documents/:documentId', async (req, res) => {
    try {
        ...
        res.json({
            ...
            url: `/uploads/documents/${path_1.default.basename(updatedDocument.filePath)}`,
...
router.delete('/documents/:documentId', async (req, res) => {
    try {
        ...
        if (document.thumbnailPath) {
            const thumbnailFullPath = path_1.default.join(__dirname, '../..', document.thumbnailPath);
            if (fs_1.default.existsSync(thumbnailFullPath)) {
                fs_1.default.unlinkSync(thumbnailFullPath);
            }
        }
