...
function isValidTooth(tooth: number): boolean {
  return validFDITeeth.includes(tooth);
}

// GET /api/clients/:clientId/notes - Listar notas de um cliente
router.get('/clients/:clientId/notes', async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/clients/:clientId/notes', async (req: AuthRequest, res: Response) => {
  try {
    const { clientId } = req.params;
...
router.put('/clients/:clientId/notes/:noteId', async (req: AuthRequest, res: Response) => {
  try {
...
router.delete('/clients/:clientId/notes/:noteId', async (req: AuthRequest, res: Response) => {
  try {
    const { clientId, noteId } = req.params;
    const companyId = req.user!.companyId;
...
router.get('/clients/:clientId/odontograma', async (req: AuthRequest, res: Response) => {
  try {
    const { clientId } = req.params;
    const companyId = req.user!.companyId;
    ...
    const odontogramNotes = await prisma.clinicalNote.findMany({
      where: {
        clientId,
        companyId,
    ...
    // Agrupar por dente
    const teethData: { [key: number]: any } = {};
...
router.post('/appointments/:id/notes', async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/clients/:clientId/history', async (req: AuthRequest, res: Response) => {
  try {
    const { clientId } = req.params;
    const companyId = req.user!.companyId;
